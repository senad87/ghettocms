...
<div class="box-right-panel">
        ...
        <select name="" class="filter">
          ...
          <?php foreach($topics as $topic) { ?>
	          <option value="<?php echo $topic->id; ?>" <?php if($topic->id == $selected_topic){?> selected="selected"<?php }?>><?php echo $topic->name; ?></option>
          <?php } ?>
        </select>
        </div>
...
<div class="submenu">
		<!-- <a class="button" href="<?=base_url(); ?>tag/new_tag/">New</a>
		<button class="button-1" id="delete_story_button">Delete</button> -->
</div>
	<!-- <table class="data" border="0" cellpadding="0" cellspacing="0">
		<thead>
		<tr>
		<th width="35" style="text-align: center;">ID</th><th style="text-align: center;" width="10"><input id="all_stories" type="checkbox" value=""></th><th width="300" align="left">Tag</th><th width="300" align="left">Topic</th>
		</tr>
		</thead>
		<?php 
		$i=0;
		foreach($tags_data as $tag) { ?>
			...
			<td align="right"><?php echo $tag['id']; ?></td><td align="center" width="20"><input type="checkbox" name="story_id" value="<?php echo $tag['id']; ?>"></td><td align="left"><a href="<?=base_url(); ?>tag/edit/<?php echo $tag['id']; ?>"><?php echo $tag['tag']; ?></a></td><td align="center"><?php echo $tag['topic_name']; ?></td>
			</tr>
		<?php 
		$i++;
		 } ?>
...
$('.filter').change(function() {
	...
	//alert("<?php echo base_url(); ?>tag/filter/"+filter_value);
