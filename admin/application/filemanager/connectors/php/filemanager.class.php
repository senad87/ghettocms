...
class Filemanager {
  ...
  private function availableLanguages() {
    ...
    if ($handle = opendir($this->root.'/scripts/languages/')) {
      while (false !== ($file = readdir($handle))) {
        if ($file != "." && $file != "..") {
        	/*** damir fix for older php version (before) ***/
        	$path_parts = pathinfo($file);
        	//var_dump($path_parts);
        	$path_parts = explode(".", $path_parts['basename']);
        	$number_of_elements = count($path_parts);
        	$filename_number = $number_of_elements - 1;
        	$filename = $path_parts[$filename_number];
          	array_push($this->languages, $filename);
          	/*** end of damir fix ***/
        	//original
